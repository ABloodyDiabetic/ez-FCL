...
@available(iOS 16, *) struct StateezFCLResults: AppEntity {
    static var defaultQuery = StateBGQuery()

    static var typeDisplayRepresentation: TypeDisplayRepresentation = "ezFCL State Result"
...
@available(iOS 16.0, *) struct StateBGQuery: EntityQuery {
    func entities(for _: [StateezFCLResults.ID]) async throws -> [StateezFCLResults] {
        []
    }

    func suggestedEntities() async throws -> [StateezFCLResults] {
        []
    }
}
